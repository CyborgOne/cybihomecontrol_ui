 ...
 * @param $w="" 
 */ 
  function DbTable($con, $tbname="", $cols=array("*"), $labels="", $defaults="", $o="", $w=""){   //Konstruktor
    ...
    $this->setWhere($w);
	$this->setOrderBy($o);
	$this->MAX_ROWS_TO_FETCH = getPageConfigParam($_SESSION['config']->DBCONNECT,"max_rowcount_for_dbtable");
    
	$this->setLimitActive(false);
    
	if(isset($_SESSION['tmp']['CURRENT_PAGE_'.$this->TABLENAME]) && strlen($_SESSION['tmp']['CURRENT_PAGE_'.$this->TABLENAME])>0){
    ...
    if(isset($_REQUEST["changeDbPage".$this->TABLENAME]) && $_REQUEST["changeDbPage".$this->TABLENAME]>0){
	  $this->CURRENT_PAGE = $_REQUEST["changeDbPage".$this->TABLENAME];
	  $_SESSION['tmp']['CURRENT_PAGE_'.$this->TABLENAME] = $_REQUEST["changeDbPage".$this->TABLENAME];	
	}
    ...
    if($this->COLNAMES[0]=="*") {
	 ...
	 if ( $fldName != "rowid" ) {$this->COLNAMES[$i] = $fldName;}
      }
    }
    // -------------------------------------------------------
...
*/
  function checkTableStructure(){
     ...
     if(!$this->existsColumn("geaendert")){
        $this->addColumn("geaendert", "TIMESTAMP","ON UPDATE CURRENT_TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP");
     }
  }
  ...
  function existsColumn($columnName){
    $sql = "select * from information_schema.COLUMNS "
          ."where TABLE_SCHEMA = '" .$this->DBCONNECT->dbname ."' " 
  ...
  function addColumn($columnName, $type="varchar(100)" , $params=""){
    if ( strpos($this->TABLENAME, ",") == false){ 
      $sql = "ALTER TABLE " .$this->TABLENAME ." ADD `".$columnName."` " .$type ." " .$params ." ";
      $result = $this->DBCONNECT->executeQuery($sql);
    }
  }
...
//----------------------------------------------------------------------
  ...
  */
  ...
  function setWhere($w){
  	...
  	if(strlen($w) > 3  &&  ((strpos($w, "=")>0 &&  strpos($w, "=") < strlen($w)) || (strpos($w, "is")>0 &&  strpos($w, "is") < strlen($w)) )  ){
      $this->WHERE = " WHERE ". $w ." ";
	}else{
	  $this->WHERE = "";	
	}
 ...
 */
  ...
  //SQL zusammenbauen für aktualisierung
    $stmt = "SELECT " .$this->COLNAMESTRING .", " .$preTabAlias ."id as rowid FROM " 
	        .$this->TABLENAME ." " .$this->getWhere() .$this->ORDERBY;
 ...
 */ 
  function getInsertMask(){
      ...
      */
        ...
        if( strlen($arrChk) == 0){   
	       ...
	       // definierte Combobox laden (wenn vorhanden)
	        ...
	        if((mysql_num_rows($lookups)==0 && !$this->isDbComboSet($this->TABLENAME, $fieldName))
			  ...
			  } else if (mysql_field_type($result, $i) == "timestamp"){
	            $o = new TextField($fieldName, $val, $fieldLen, $maxLen);
	         ...
	         if(mysql_num_rows($lookups)>0){
	           $o = new LookupCombo($_SESSION['config']->DBCONNECT, $fieldName, $this->TABLENAME, $fieldName);
	         } else if(count($dbCombo)>0) {
      ...
      $hidden = new Hiddenfield("dbTableNew", "-");
 ...
 */
  ...
  function doInsert(){
     ...
     $stmnt .= " FROM " .$this->TABLENAME ." "  .$this->getWhere() .$this->ORDERBY . " LIMIT 1 ";
     $result = $this->DBCONNECT->executeQuery($stmnt);
...
*/
     ...
     for($i=0; $i<mysql_num_fields($result); $i++){
	   ...
	   if((isset($fieldValue) && trim(strlen($fieldValue))>0 )  || 
	      (count($ev)==2 && (in_array('J', $ev) && in_array('N', $ev)) )){
	       ...
	       } else if (strpos(mysql_field_flags($result, $i), "enum")>0){
     		 $ev = $this->getEnumValues($fieldName);
		     if(count($ev)==2 && (in_array('J', $ev) && in_array('N', $ev)) ){
                //Checkbox braucht Sonderbehandlung, da bei Wert=N  kein Wert übergeben wird!
                if( isset($fieldValue)){
...
//----------------------------------------------------------------------
//  LÃƒÂƒÃ‚ÂƒÃƒÂ‚Ã‚ÂƒÃƒÂƒÃ‚Â‚ÃƒÂ‚Ã‚Â–SCHEN - MASKE und Verarbeitung
//----------------------------------------------------------------------
...
//----------------------------------------------------------------------
  ...
  function doDelete(){
     ...
     for ( $ir=1;  $ir<=count($this->ROWS);  $ir++ ){
       ...
       if( isset($_REQUEST[$this->ROWS[$ir]->getNamedAttribute("id")] ) && $_REQUEST[$this->ROWS[$ir]->getNamedAttribute("id")] == "entfernen" ){
...
//----------------------------------------------------------------------
  ...
  function  showUpdateMask(){
    if( isset($_REQUEST['DbTableUpdate'.$this->TABLENAME]) && $_REQUEST['DbTableUpdate'.$this->TABLENAME] == "Speichern" ){
	  $this->doUpdate();
    }
  ...
  function isDbComboSet($tab, $col){
    $sql = "SELECT * FROM dbcombos where tab_name = '" .$tab ."' AND col_name = '" .$col ."' limit 1";
  ...
  function  getUpdateMask(){
    ...
    for ( $ir=1;  $ir<=count($this->ROWS);  $ir++ ){
      ...
      for ($ia=0; $ia<count($this->COLNAMES);  $ia++ ){ 
        ...
        } else if (mysql_field_type($result, $ia) == "date"){
         ...
         if($this->isDbComboSet($this->TABLENAME, $fieldName)){
		   ...
		   if(count($dbCombo)>0) {
  	       ...
  	       if (mysql_field_type($result, $ia) == "blob"){
			  ...
			  } else if (mysql_field_type($result, $ia) == "int"){
			  ...
			  } else if (mysql_field_type($result, $ia) == "timestamp"){
                $t = new TextField($fieldName.$rowId,$row->getNamedAttribute($colNames[$ia]),  $fieldLen, $maxLen );
        ...
        if( strlen($arrChk) == 0){   
	       ...
	       // definierte Combobox laden (wenn vorhanden)
			...
			if(mysql_num_rows($lookups)==0 && !$this->isDbComboSet($this->TABLENAME, $fieldName)){
	         ...
	         if(mysql_num_rows($lookups)>0){
	           $o = new LookupCombo($_SESSION['config']->DBCONNECT, $fieldName.$rowId, $this->TABLENAME, $fieldName);
	         } else if(count($dbCombo)>0 ) {
                   if(!strpos(" ".$this->DEFAULTS, $fieldName)>0){
...
//----------------------------------------------------------------------
  ...
  function insertRowByArray($array){
	...
	function getEnumValues($field){
		...
		$e = new Error("Fehler in getEnumValues()", "Fehler beim holen der Enum-Werte für  Feld {$field} aus Tabelle {$this->TABLENAME}");
	}
