...
class ImageUploader extends Object {
    ...
    // Wenn gefÃÂÃÂÃÂÃÂ¼llt, wird in die entsprechende Row der Tabelle in die angegebene Spalte
    ...
    function ImageUploader($targetPath, $filePrefix = "", $dbInsertTable = "", $dbInsertColumn =
        "", $dbInsertId = "", $hiddenObj = "", $targetFileName = "") {
    ...
    function setTargetPath($tp) {
        $f = $_SERVER['DOCUMENT_ROOT'] . $tp;

        if (file_exists($tp)) {
            $this->TARGET_PATH = $tp;
        } else {
            if (file_exists($_SERVER['DOCUMENT_ROOT'] . $tp)) {
                $this->TARGET_PATH = $tp;
            } else {
                $m = new Error("Fehlerhaftes Verzeichniss", "Verzeichniss *" . $tp .
                    "* existiert nicht!");
            }
        }
    ...
    /**
     * diese Methode muss ÃÂÃÂÃÂÃÂ¼berschrieben werden, 
     * wenn nach dem Hochladen der Datei noch eine weitere Aktion ausgefÃÂÃÂÃÂÃÂ¼hrt werden soll.
     * 
     * Informationen zur hochgeladenen Datei erhält man mittels  $_FILES['probe']
     * 
     * Bspl:
     * $_FILES['probe']['name']
    ...
    /**
     * trägt den Dateinamen mit relativer Pfadangabe in die angegebene Tabelle/Spalte/Zeile ein, 
     * wenn die entsprechenden 3 DbInsert... Werte angegeben wurden.
     */
    function insertIntoDbIfNeeded($newFileName) {

        if (strlen($this->DB_INSERT_TABLE) > 0 && strlen($this->DB_INSERT_COLUMN) > 0 &&
            strlen($this->DB_INSERT_ID) > 0) {

            $sql = "UPDATE " . $this->DB_INSERT_TABLE . "   SET " . $this->DB_INSERT_COLUMN .
                " = '" . $newFileName . "' " . " WHERE id = " . $this->DB_INSERT_ID;
    ...
    function showFileUploadMask() {
        ...
        $btn = new Button("upload", "Datei hochladen",
            "alert('Datei wird hochgeladen.Dieser Vorgang kann einige Zeit in Anspruch nehmen!Nach Abschluss erhalten Sie eine Bestätigung des Upload-Vorgangs.Brechen Sie nicht vorher Ab!')");
    ...
    /**
     * Dateiupload-Vorgang ausfÃÂÃÂÃÂÃÂ¼hren
     */
    function doUploadFile() {
        //Relative Pfadangabe fÃÂÃÂÃÂÃÂ¼r Upload-Verzeichniss ermitteln
        ...
        $newFileName = $folder->doUploadFile($_FILES['probe'], substr($folder->getPath(),
            strlen($_SERVER['DOCUMENT_ROOT'])), $this->getFilePrefix(), $this->
            TARGETFILENAME);
    ...
    /**
     * Zeigt je nach bedarf die Datei-Auswahl an oder fÃÂÃÂÃÂÃÂ¼hrt den Upload-Vorgang durch.
     */
    function show() {
        if (!isset($_REQUEST['picImportUpload']) || (isset($_REQUEST['picImportUpload']) &&
            $_REQUEST['picImportUpload'] != "doUpload")) {
            //Solange der Upload-Dialog nicht bestätigt wurde, Upload-Maske anzeigen
            ...
            if (isset($_FILES['probe'])) {
                ...
                //Fehlt die Berichts-Id, diese auswählen und Datei hochladen.
                if (isset($_FILES['probe'])) {
                    $e = new Error("Datei nicht gefunden", "Keine Datei zum hochladen gefunden. ");
                }

                $e = new Error("Unerwarteter Fehler",
                    "Es ist ein unerwarteter Fehler aufgetreten. \n\nSollte der Fehler wieder auftauchen informieren Sie uns bitte ÃÂÃÂÃÂÃÂ¼ber das Kontaktformular. ");
            }
    }
