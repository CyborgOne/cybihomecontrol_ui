...
$timelineDuration = getPageConfigParam($_SESSION['config']->DBCONNECT,
    "timelineDuration");
...
$w = "(tagnr=" . $dayOfWeek . " AND ((stunde >= " . date("H") . " AND minute > " .
    date("i") . " ) OR (stunde > " . date("H") . "))) OR ";

$o = "if( ((tagnr=" . $dayOfWeek . " AND ((stunde >= " . date("H") .
    " AND minute > " . date("i") . " ) OR (stunde > " . date("H") .
    "))) OR (tagnr!=" . $dayOfWeek . ") ) AND tagnr-" . ($dayOfWeek) . ">=0 ,tagnr-" . ($dayOfWeek) .
    ", tagnr+(7-" . ($dayOfWeek) . ")), stunde, minute";
...
$w .= " (tagnr=" . $nextDayOfWeek . " AND ((stunde <= " . date("H") .
    " AND minute <= " . date("i") . " ) OR " . "(stunde < " . date("H") . ")))";


$scDbTable = new DbTable($_SESSION['config']->DBCONNECT, 'homecontrol_cronview',
    array("wochentag", "tagnr", "name", "montag", "dienstag", "mittwoch",
    "donnerstag", "freitag", "samstag", "sonntag", "stunde", "minute", "id"),
...
foreach ($scDbTable->ROWS as $row) {
    ...
    $executionTime = new Text($cron->getNextExecutionTimeAsString(), 5);
    $name = new Text($cron->getName(), 5);
...
if ($_SESSION['config']->CURRENTUSER->STATUS != "admin" && $_SESSION['config']->
    CURRENTUSER->STATUS != "user" && $loginNeed) {
