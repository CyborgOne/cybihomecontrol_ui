...
// --------------------------------------------------
    ...
    $cobSelect = new ComboBoxBySql($_SESSION['config']->DBCONNECT, "SELECT id, name FROM homecontrol_sensor ORDER BY name", "SelectedSensorToEdit", isset($_SESSION['SelectedSensorToEdit'])?$_SESSION['SelectedSensorToEdit']:"", "id", "name", " ");
    $cobSelect->setDirectSelect(true);

    $rAuswahl = $table->createRow();
    $rAuswahl->setColSizes(array(120));
    $rAuswahl->setAttribute(0, new Text("Sensor auswählen: "));
...
// Zuordnung ausgewählt
    ...
    if( isset($_SESSION['SelectedSensorToEdit']) && strlen($_SESSION['SelectedSensorToEdit'])>0 ){
        $scItemsDbTable  = new DbTable($_SESSION['config']->DBCONNECT,
                                       'homecontrol_sensor_items', 
	                                   array("config_id", "art_id", "zimmer_id", "etagen_id", "on_off", "sensor_id" ) , 
                                       "Objekt, Objekt-Art, Zimmer, Etage, An/Aus",
                                       "sensor_id=".$_SESSION['SelectedSensorToEdit'],
        ...
        // Neuer Eintrag
        if(isset($_REQUEST['InsertIntoDBhomecontrol_sensor_items']) && $_REQUEST['InsertIntoDBhomecontrol_sensor_items']=="Speichern"){

            $scItemsDbTable->doInsert();
            $scItemsDbTable->refresh();

        } else if(isset($_REQUEST['dbTableNew_Sensor_items']) ) {      
    ...
    $newItemBtn = $scDbTable->getNewEntryButton("Neuen Eintrag", "_Sensor_items");
