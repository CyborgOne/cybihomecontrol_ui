 ...
 * DB Comments!
 */
function getTermWhereclauseForConfigId($configId){
    // SELECT * FROM homecontrol_config WHERE ...
    $WHERE = "";
    $term_type = 0;
    
    $result = $_SESSION['config']->DBCONNECT->executeQuery("SELECT * FROM homecontrol_term WHERE config_id=".$configId." ORDER BY order_nr");
...
function getWhereclauseForSensorWert($sensorwert, $termRow){
    return $sensorwert." ".$termRow['condition']." ".$termRow['value']." ";
}
...
function getWhereclauseForSensorStatus($status, $termRow){
    ...
    if(strlen($termRow['value'])>0){
        $w = $w ." AND " .(time() - $termRow['value']) 
            ." > (SELECT lastSignal FROM homecontrol_sensor WHERE id = " 
            .$termRow['sensor_id'] .")";
    }
...
function getWhereclauseForTime($time, $termRow){
    return $time." ".$termRow['condition']." ".$termRow['min'].$termRow['std'];
}
