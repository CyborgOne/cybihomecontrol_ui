...
mysqldump -d  --compatible=mysql40 --skip-comments --skip-set-charset --skip-tz-utc -u root -p$databasePasswd -h $databaseHost $databaseName homecontrol_sender_typen				| sed -e 's/TYPE=MyISAM/ /g' | sed -e 's/TYPE=InnoDB/ /g' >> $filename
mysqldump -d  --compatible=mysql40 --skip-comments --skip-set-charset --skip-tz-utc -u root -p$databasePasswd -h $databaseHost $databaseName homecontrol_sender_typen_parameter			| sed -e 's/TYPE=MyISAM/ /g' | sed -e 's/TYPE=InnoDB/ /g' >> $filename
mysqldump -d  --compatible=mysql40 --skip-comments --skip-set-charset --skip-tz-utc -u root -p$databasePasswd -h $databaseHost $databaseName homecontrol_sender_typen_parameter_arten		| sed -e 's/TYPE=MyISAM/ /g' | sed -e 's/TYPE=InnoDB/ /g' >> $filename
